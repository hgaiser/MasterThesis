 ...
 * Here's the complete std::list of references:
...
class DeviceMemoryPool
{
    public:
            ...
            chunkSize_(chunkSize)
        {
            ...
            {
                std::cout << "Pool memory allocation failed (" << e.what() << ")"
                     << std::endl;
                exit(EXIT_FAILURE);
            }
        ...
        // Pushes an address stored in "ptr" to the std::list
    ...
    private:
        ...
        std::list< thrust::device_ptr<T> > chunks_;
};
...
struct Graph
{
    ...
    // This std::vector stores offsets for each vertex in "edges" and "weights"
    // std::vectors. For example:
    ...
    // This std::vector stores indices of endpoints of the corresponding edges.
    // For example, "edges[vertices[0]]" is the first neighbouring vertex
    // of vertex #0.
    std::vector<uint> edges;

    // This std::vector stores weights of the corresponding edges.
...
class Pyramid
{
    public:
        ...
        std::vector<cv::Mat> getLevels(uint width, uint height) const {
			uint levelIndex = 0;
			...
			for (LevelsIterator level = levels_.rbegin();
				 ...
				 ++level, ++levelIndex)
			{
				levels.push_back(getLevel(level, width, height));
			}
        ...
        void dump(uint width, uint height) const
        {
            ...
            uint requiredDigitsCount =
                ...
                1;
            sprintf(format, "level_%%0%uu.png", requiredDigitsCount);
    ...
    private:
        ...
        class Level
        {
            ...
            private:
                ...
                // The pair of the following std::vectors describes the
                ...
                std::vector<uint> superNodesOffsets_;
                std::vector<uint> nodes_;
        };

        typedef std::list<Level>::const_reverse_iterator LevelsIterator;

		cv::Mat getLevel(LevelsIterator level, uint width, uint height) const {
			std::deque< std::pair<uint, uint> > nodesQueue;

			{
				const std::vector<uint> &superNodesOffsets =
			...
			while (!nodesQueue.empty())
			{
				...
				image.at<int>(pixelIndex) = pixelSegment;
			}
                       ...
                       const char *filename) const
        {
            ...
            std::vector<uint> colors(3 * totalSegments);
            ...
            while (!nodesQueue.empty())
            {
                ...
                image.at<cv::Vec3b>(pixelIndex)[0] = colors[pixelSegment * 3    ];
                image.at<cv::Vec3b>(pixelIndex)[1] = colors[pixelSegment * 3 + 1];
                image.at<cv::Vec3b>(pixelIndex)[2] = colors[pixelSegment * 3 + 2];
            }
...
class SegmentationTreeBuilder
{
    public:
        ...
        float run(const Graph &graph, Pyramid &segmentations)
        {
            ...
            {
                std::cout << "Algorithm failed (" << e.what() << ")" << std::endl;
                exit(EXIT_FAILURE);
            }
...
inline float distance(const cv::Vec3b & first, const cv::Vec3b & second)
{
    int dx = static_cast<int>(first[0]) - static_cast<int>(second[0]);
    int dy = static_cast<int>(first[1]) - static_cast<int>(second[1]);
    int dz = static_cast<int>(first[2]) - static_cast<int>(second[2]);
...
void buildGraph(const cv::Mat & image,
                ...
                Graph &graph)
{
    uint totalNodes = static_cast<uint>(image.total());
    ...
    for (uint y = 0; y < height; ++y)
    {
        for (uint x = 0; x < width; ++x)
        {
            ...
            const cv::Vec3b & centerPixel = image.at<cv::Vec3b>(nodeIndex);
            ...
            if (y > 0)
            {
                ...
                const cv::Vec3b & lowerPixel = image.at<cv::Vec3b>(lowerNodeIndex);
            ...
            if (y + 1 < height)
            {
                ...
                const cv::Vec3b & upperPixel = image.at<cv::Vec3b>(upperNodeIndex);
            ...
            if (x > 0)
            {
                ...
                const cv::Vec3b & leftPixel = image.at<cv::Vec3b>(leftNodeIndex);
            ...
            if (x + 1 < width)
            {
                ...
                const cv::Vec3b & rightPixel = image.at<cv::Vec3b>(rightNodeIndex);
...
bool startCuda() {
	...
	if ((deviceProp.major > 1) ||
		...
		std::cerr << "SegmentationTree requires a CUDA device with compute capability 1.3 or higher." << std::endl;
		stopCuda();
		return false;
	}
}
...
std::vector<cv::Mat> computeSegmentationLevels_(const cv::Mat & image) {
	Graph graph;
	buildGraph(image, image.cols, image.rows, graph);

	Pyramid segmentations;

	//std::cout << "* Building segmentation tree... ";
	//std::cout.flush();

	SegmentationTreeBuilder algo;
	float elapsedTime = algo.run(graph, segmentations);

	//std::cout << "done in " << elapsedTime << " (ms)" << std::endl;

	//std::cout << "* Dumping levels for each tree..." << std::endl << std::endl;

	return segmentations.getLevels(image.cols, image.rows);
}
...
int main(int argc, char *argv[])
{
	...
	for (int i = 0; i < levels.size(); i++) {
		cv::Mat im;
		cv::normalize(levels[i], im, 0, 255, cv::NORM_MINMAX, CV_8UC1);
		cv::imshow("Image", im);
		cv::waitKey();
	}
